...
function configureBridgeOptions(options) {
  ...
  if (options.dtlsUpdatesPerSecond == null) {
    ...
    throw new Error(
      "PHEA: 'options.dtlsUpdatesPerSecond' must be of type int between " +
        phea_config_1.Config.DTLS_UPDATES_PER_SECOND_MIN +
        " and " +
        phea_config_1.Config.DTLS_UPDATES_PER_SECOND_MAX +
        " inclusive."
    );
  }
  if (options.colorUpdatesPerSecond == null) {
    ...
    typeof options.colorUpdatesPerSecond !== "number" ||
    ...
    throw new Error(
      "PHEA: 'colorUpdatesPerSecond' must be of type int between " +
        phea_config_1.Config.COLOR_UPDATES_PER_SECOND_MIN +
        " and " +
        phea_config_1.Config.COLOR_UPDATES_PER_SECOND_MAX +
        " inclusive."
    );
  }
  if (options.dtlsPort == null) {
    ...
    typeof options.dtlsPort !== "number" ||
    options.dtlsPort < phea_config_1.Config.DTLS_PORT_MIN ||
    options.dtlsPort > phea_config_1.Config.DTLS_PORT_MAX
  ) {
    throw new Error(
      "PHEA: 'dtlsPort' must be of type int between " +
        phea_config_1.Config.DTLS_PORT_MIN +
        " and " +
        phea_config_1.Config.DTLS_PORT_MAX +
        " inclusive."
    );
  }
  if (options.dtlsTimeoutMs == null) {
    ...
    typeof options.dtlsTimeoutMs !== "number" ||
    options.dtlsTimeoutMs < phea_config_1.Config.DTLS_TIMEOUT_MIN ||
    options.dtlsTimeoutMs > phea_config_1.Config.DTLS_TIMEOUT_MAX
  ) {
    throw new Error(
      "PHEA: 'dtlsTimeout' must be of type int between " +
        phea_config_1.Config.DTLS_TIMEOUT_MIN +
        " and " +
        phea_config_1.Config.DTLS_TIMEOUT_MAX +
        " inclusive."
    );
  }
}
